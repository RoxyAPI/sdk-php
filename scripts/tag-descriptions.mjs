...
export function tagToNamespace(name) {
	return NAMESPACE_ALIASES[name] ?? camelize(name);
}

export function tagToClassName(name) {
	const ns = tagToNamespace(name);
	return ns[0].toUpperCase() + ns.slice(1) + 'Resource';
}
...
export function tagSummary(tag) {
	if (tag?.['x-sdk-summary']) return tag['x-sdk-summary'].trim();
